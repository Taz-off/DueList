...
const state = {
  ...
  tasks: [],
  archivedTasks: [],
  selectedArchiveTaskIds: new Set(),
  quickNotes: [],
  subjects: [],
  todayTasks: [],
  upcomingTasks: [],
...
const routes = ["home", "agenda", "add", "notes", "archive", "settings"];
...
async function startApp() {
  ...
  try {
    if (!window.DueListDB || typeof initDB !== "function") {
      throw new Error("La couche de données DueList n'a pas été chargée correctement.");
    }
...
function bindNavigation() {
  ...
  document.body.addEventListener("change", async (event) => {
    if (event.target.matches("#dark-mode-toggle, [data-action='update-subject-color']")) {
...
async function loadTaskCollections() {
  ...
  const subjectNames = [...tasks, ...archivedTasks].map((task) => task.subject);
  state.tasks = tasks;
  state.subjects = mergeSubjectRecords(subjects, subjectNames);
    ...
    <section class="home-view" aria-labelledby="home-title">
      <header class="page-header">
        ...
        <div class="header-actions">
          <button class="secondary-button" type="button" data-route="archive">Archive</button>
          <button class="primary-button" type="button" data-route="add">Ajouter un devoir</button>
        </div>
      </header>

      <div class="summary-grid" aria-label="Résumé">
        ${renderSummaryCard("Aujourd'hui", state.todayTasks.length)}
        ${renderSummaryCard("À venir", state.upcomingTasks.length)}
        ${renderSummaryCard("Matières", state.subjects.length)}
        ${renderSummaryCard("Archivés", state.archivedTasks.length)}
      </div>
...
function renderEvaluationItem(task) {
  ...
  return `
    <article class="evaluation-item" style="--subject-color: ${escapeAttribute(subjectColor)}">
      <span class="date-chip">${formatDayNumber(task.dueDate)}</span>
      <div>
        <strong><span class="subject-dot"></span>${escapeHTML(task.subject)}</strong>
      ...
      <div class="settings-layout">
        <section class="settings-panel" aria-label="Préférences">
          <div class="settings-heading">
            <span class="settings-icon" aria-hidden="true">${settingsIcon()}</span>
        ...
        <section class="settings-panel" aria-labelledby="subject-colors-title">
          <div class="settings-heading">
            <span class="settings-icon" aria-hidden="true">${paletteIcon()}</span>
            <strong id="subject-colors-title">Couleurs des matières</strong>
          </div>
          ${renderSubjectColorManager()}
        </section>

        <section class="settings-panel" aria-labelledby="data-title">
          <div class="settings-heading">
            <span class="settings-icon" aria-hidden="true">${databaseIcon()}</span>
            <strong id="data-title">Données</strong>
          </div>
          <div class="settings-actions">
            <button class="secondary-button" type="button" data-action="export-data">Exporter les données</button>
            <label class="file-field">
              <span>Fichier JSON</span>
              <input id="import-file" type="file" accept="application/json,.json">
            </label>
            <div class="segmented-control" role="radiogroup" aria-label="Mode d'import">
              <label>
            ...
            <button class="primary-button" type="button" data-action="import-data">Importer des données</button>
          </div>
        </section>

        <section class="settings-panel danger-zone" aria-labelledby="reset-title">
          <div class="settings-heading">
            <span class="settings-icon" aria-hidden="true">${resetIcon()}</span>
            <strong id="reset-title">Réinitialisation</strong>
          </div>
...
function renderSubjectColorManager() {
  if (!state.subjects.length) {
    return `
      <div class="empty-state compact">
        <strong>Aucune matière</strong>
        <span>Les matières ajoutées apparaîtront ici.</span>
      </div>
    `;
  }

  return `
    <div class="subject-color-list">
      ${state.subjects.map((subject) => {
        return `
          <label class="subject-color-row">
            <span class="subject-swatch" style="--subject-color: ${escapeAttribute(subject.color)}"></span>
            <strong>${escapeHTML(subject.name)}</strong>
            <input
              type="color"
              value="${escapeAttribute(subject.color)}"
              data-action="update-subject-color"
              data-subject="${escapeAttribute(subject.name)}"
              aria-label="Couleur ${escapeAttribute(subject.name)}"
            >
          </label>
        `;
      }).join("")}
...
function renderCalendarDay(day) {
  ...
  const tasksForDay = state.tasks.filter((task) => task.dueDate === date);
  ...
  return `
    <button class="${classes}" type="button" data-action="select-date" data-date="${date}" aria-pressed="${isSelected}">
      ...
      <span class="calendar-indicators" aria-hidden="true">
        ${renderCalendarSubjectDots(tasksForDay)}
...
function renderCalendarSubjectDots(tasks) {
  const colors = [];
  ...
  return colors
    .slice(0, 4)
    .map((color) => `<span class="subject-dot" style="--subject-color: ${escapeAttribute(color)}"></span>`)
    .join("");
}
      ...
      <form class="form-panel" id="task-form" novalidate>
        ...
        <div class="form-grid">
          ...
          <label class="field color-field">
            ...
            <input id="subject-color-input" type="color" name="subjectColor" value="${escapeAttribute(subjectColor)}">
          </label>
...
function bindTaskForm(task) {
  ...
  const subjectColorInput = document.querySelector("#subject-color-input");
  ...
  subjectInput.addEventListener("input", () => {
    subjectColorInput.value = getSubjectColor(subjectInput.value);
  });
...
function renderSubjectOptions() {
  return state.subjects
    .map((subject) => `<option value="${escapeAttribute(subject.name)}"></option>`)
    .join("");
}
...
async function saveTaskFromForm(form, existingTask) {
  ...
  const subjectColor = normalizeColor(formData.get("subjectColor")) || getSubjectColor(subject);
  ...
  await updateSubjectColor(subject, subjectColor);
...
function renderTaskCard(task, options = {}) {
  ...
  const archiveClass = options.archive ? "is-archive" : "";
  const subjectColor = getSubjectColor(task.subject);
  ...
  return `
    <article class="task-card ${compactClass} ${archiveClass}" data-task-id="${escapeAttribute(task.id)}" style="--subject-color: ${escapeAttribute(subjectColor)}">
      ${options.archive ? renderArchiveSelectionControl(task) : ""}
      <div class="task-main">
        <div>
          <p class="task-subject"><span class="subject-dot"></span>${escapeHTML(task.subject)}</p>
          <h3>${escapeHTML(task.title)}</h3>
          ${description}
        </div>
        ${options.archive ? renderArchiveReason(task) : `
      ...
      <div class="task-meta">
        ...
        ${options.archive && task.archivedAt ? `<span class="meta-pill">Archivé le ${formatDateTime(task.archivedAt)}</span>` : ""}
      </div>
      <div class="task-actions">
        ${options.archive ? `
          <button class="text-button" type="button" data-action="restore-task" data-id="${escapeAttribute(task.id)}">Restaurer</button>
          <button class="text-button danger" type="button" data-action="purge-task" data-id="${escapeAttribute(task.id)}">Supprimer définitivement</button>
        ` : `
...
function renderArchiveSelectionControl(task) {
  const isSelected = state.selectedArchiveTaskIds.has(task.id);
...
function renderArchiveReason(task) {
  const label = archiveReasonLabels[task.archiveReason] || "Archivé";
  return `<span class="archive-reason">${escapeHTML(label)}</span>`;
}

function renderArchive() {
  const selectedCount = state.selectedArchiveTaskIds.size;
  const allSelected = state.archivedTasks.length > 0 && selectedCount === state.archivedTasks.length;
  ...
  app.innerHTML = `
    <section class="archive-view" aria-labelledby="archive-title">
      ...
      <section class="content-section" aria-label="Devoirs archivés">
        <div class="section-heading">
          <div>
            <p class="section-kicker">Terminés et supprimés</p>
            <h2>Devoirs archivés</h2>
          </div>
          <span class="section-count">${state.archivedTasks.length}</span>
        </div>
        ${state.archivedTasks.length ? `
          <div class="archive-toolbar" aria-label="Actions de sélection">
            <span>${selectedCount} sélectionné(s)</span>
            <div>
              <button class="secondary-button small-button" type="button" data-action="toggle-all-archive-selection">
                ${allSelected ? "Tout désélectionner" : "Tout sélectionner"}
              </button>
              <button class="secondary-button small-button danger-button" type="button" data-action="purge-selected-archive" ${selectedCount ? "" : "disabled"}>
...
function renderArchiveTaskList() {
  if (!state.archivedTasks.length) {
    return `
      <div class="empty-state">
        <strong>Archive vide</strong>
        <span>Les devoirs terminés ou supprimés apparaîtront ici.</span>
      </div>
    `;
  }

  return renderTaskList(state.archivedTasks, "", { archive: true });
}

function syncArchiveSelection() {
  const archivedIds = new Set(state.archivedTasks.map((task) => task.id));
  state.selectedArchiveTaskIds = new Set(
    [...state.selectedArchiveTaskIds].filter((taskId) => archivedIds.has(taskId))
  );
}
...
async function handleAction(button) {
  ...
  if (action === "update-subject-color") {
    const subject = button.dataset.subject;
    const color = normalizeColor(button.value);
    await updateSubjectColor(subject, color);
    state.subjects = state.subjects.map((item) => {
      return item.name === subject ? { ...item, color } : item;
    });
  ...
  if (action === "complete-task") {
    ...
    await loadTaskCollections();
    showToast("Devoir déplacé dans l'archive.");
    render();
    return;
  }
  ...
  if (action === "delete-task") {
    const confirmed = window.confirm("Déplacer ce devoir dans l'archive ?");

    if (confirmed) {
      await deleteTask(button.dataset.id);
      await loadTaskCollections();
      showToast("Devoir archivé.");
      render();
    }
    return;
  }

  if (action === "toggle-archive-selection") {
    const taskId = button.dataset.id;

    if (button.checked) {
      state.selectedArchiveTaskIds.add(taskId);
    } else {
      state.selectedArchiveTaskIds.delete(taskId);
    }
  ...
  if (action === "toggle-all-archive-selection") {
    const allSelected = state.archivedTasks.length > 0 && state.selectedArchiveTaskIds.size === state.archivedTasks.length;
    state.selectedArchiveTaskIds = allSelected
      ? new Set()
      : new Set(state.archivedTasks.map((task) => task.id));
    renderArchive();
    return;
  }

  if (action === "restore-task") {
    await restoreTask(button.dataset.id);
    state.selectedArchiveTaskIds.delete(button.dataset.id);
    await loadTaskCollections();
    showToast("Devoir restauré.");
    render();
    return;
  }

  if (action === "purge-task") {
    const confirmed = window.confirm("Supprimer définitivement ce devoir ? Cette action est irréversible.");

    if (confirmed) {
      await permanentlyDeleteTask(button.dataset.id);
      state.selectedArchiveTaskIds.delete(button.dataset.id);
      await loadTaskCollections();
      showToast("Devoir supprimé définitivement.");
      render();
    }
    return;
  }

  if (action === "purge-selected-archive") {
    const selectedIds = [...state.selectedArchiveTaskIds];

    if (!selectedIds.length) {
      return;
    }

    const confirmed = window.confirm(`Supprimer définitivement ${selectedIds.length} devoir(s) de l'archive ? Cette action est irréversible.`);

    if (confirmed) {
      await Promise.all(selectedIds.map((taskId) => permanentlyDeleteTask(taskId)));
      state.selectedArchiveTaskIds.clear();
      await loadTaskCollections();
  ...
  if (action === "import-data") {
    await importDataFromSettings();
    return;
  }

  if (action === "reset-app") {
    const confirmed = window.confirm("Réinitialiser DueList ? Tous les devoirs, archives, matières et notes seront supprimés.");
...
async function saveSubjectFromCurrentForm() {
  ...
  const color = normalizeColor(subjectColorInput?.value) || getSubjectColor(subject);
  ...
  await addSubject(subject, color);
  state.subjects = mergeSubjectRecords(state.subjects, [subject]).map((item) => {
    return item.name === subject ? { ...item, color } : item;
  });
...
async function importDataFromSettings() {
  ...
  if (!file) {
    showToast("Choisis un fichier JSON à importer.");
    return;
  }
  ...
  const confirmed = window.confirm(
    mode === "replace"
      ? "Remplacer les données actuelles par ce fichier JSON ?"
      : "Fusionner ce fichier JSON avec les données actuelles ?"
  );
  ...
  try {
    ...
    const result = await importAppData(payload, mode);
    state.theme = await getAppTheme();
    applyTheme(state.theme);
    await loadTaskCollections();
    showToast(`${result.taskCount} devoir(s) importé(s).`);
    render();
  } catch (error) {
    showToast(error.message || "Import JSON invalide.");
  }
}
...
function downloadJSON(data) {
  ...
  link.href = url;
  link.download = `duelist-export-${fileDate}.json`;
...
function applyTheme(theme) {
  ...
  const logoPath = nextTheme === "dark" ? "./public/assets/logos/logo-dark.png" : "./public/assets/logos/logo-light.png";
  ...
  if (favicon) {
    favicon.setAttribute("href", "./public/assets/logos/logo-light.png");
  }
...
function mergeSubjectRecords(subjectRecords, subjectNames) {
  const subjectMap = new Map();

  subjectRecords.forEach((subject) => {
    const cleanName = String(subject.name || "").trim();

    if (!cleanName) {
      return;
    }

    subjectMap.set(getSubjectKey(cleanName), {
      name: cleanName,
      color: normalizeColor(subject.color) || getDefaultSubjectColor(cleanName)
    });
  });

  subjectNames.forEach((subjectName) => {
    const cleanName = String(subjectName || "").trim();
    const subjectKey = getSubjectKey(cleanName);

    if (!cleanName || subjectMap.has(subjectKey)) {
      return;
    }

    subjectMap.set(subjectKey, {
      name: cleanName,
      color: getDefaultSubjectColor(cleanName)
    });
  });

  return [...subjectMap.values()].sort((firstSubject, secondSubject) => {
    return firstSubject.name.localeCompare(secondSubject.name, "fr");
  });
}

function getSubjectColor(subjectName) {
  const cleanName = String(subjectName || "").trim();
  const subject = state.subjects.find((item) => getSubjectKey(item.name) === getSubjectKey(cleanName));
  return normalizeColor(subject?.color) || getDefaultSubjectColor(cleanName);
}

function getSubjectKey(subjectName) {
  return String(subjectName || "").trim().toLocaleLowerCase("fr");
}

function getDefaultSubjectColor(name) {
  const text = String(name || "Matière");
  let hash = 0;

  for (let index = 0; index < text.length; index += 1) {
    hash = (hash + text.charCodeAt(index) * (index + 1)) % defaultSubjectColors.length;
  }

  return defaultSubjectColors[hash];
}
...
function paletteIcon() {
  return `
    <svg viewBox="0 0 24 24" focusable="false">
      <path d="M12 4a8 8 0 0 0 0 16h1.3a1.7 1.7 0 0 0 1.2-2.9 1.7 1.7 0 0 1 1.2-2.9H17a3 3 0 0 0 0-6h-1.1A8 8 0 0 0 12 4Z"></path>
      <path d="M7.8 11.2h.01M10.2 8h.01M14.3 8.2h.01"></path>
    </svg>
  `;
}
...
function databaseIcon() {
  return `
    <svg viewBox="0 0 24 24" focusable="false">
      <path d="M5 6.5C5 4.6 8.1 3 12 3s7 1.6 7 3.5S15.9 10 12 10 5 8.4 5 6.5Z"></path>
      <path d="M5 6.5v5C5 13.4 8.1 15 12 15s7-1.6 7-3.5v-5"></path>
      <path d="M5 11.5v5C5 18.4 8.1 20 12 20s7-1.6 7-3.5v-5"></path>
    </svg>
  `;
}
